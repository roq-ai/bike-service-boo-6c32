...
import { useGarageFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'garage-manager', 'mechanic'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useGarageFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Garages',
      path: isTenantUser ? `/garages/view/${data?.id}` : '/garages',
      entity: 'garage',
    ...
    {
      name: 'Garage Managers',
      path: '/garage-managers',
      entity: 'garage_manager',
      service: AccessServiceEnum.PROJECT,
      icon: FiClipboard,
    },
    { name: 'Mechanics', path: '/mechanics', entity: 'mechanic', service: AccessServiceEnum.PROJECT, icon: FiTool },
