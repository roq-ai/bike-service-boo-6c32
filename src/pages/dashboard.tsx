...
import { useBusinessOwnerCount, useGarageManagerCount, useCustomerCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const garageManagerStat = useGarageManagerCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Garage Managers',
      result: garageManagerStat?.data,
      isLoading: garageManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Customers',
