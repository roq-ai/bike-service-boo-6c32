...
import { useRoqClient, useMechanicFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<MechanicInterface, unknown>;

interface MechanicListPageProps {
...
export function MechanicListPage(props: MechanicListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['specialization.contains', 'availability.contains'],
          ...params.filters,
        },
        'mechanic',
      ),
  ...
  const { data, error, isLoading, mutate } = useMechanicFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.mechanic.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: MechanicInterface) => {
    if (hasAccess('mechanic', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/mechanics/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/mechanics/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('mechanic', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/mechanics/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('mechanic', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="mechanic" />
        </Flex>

        {hasAccess('mechanic', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/mechanics/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
