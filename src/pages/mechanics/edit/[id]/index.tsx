...
import { useRoqClient, useMechanicFindFirst } from 'lib/roq';
...
function MechanicEditPage() {
  ...
  const { data, error, isLoading, mutate } = useMechanicFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: MechanicInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.mechanic.update({
        data: values as RoqTypes.mechanic,
        where: {
          id,
        },
      });
