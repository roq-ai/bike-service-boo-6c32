...
import { useRoqClient, useGarageFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<GarageInterface, unknown>;

interface GarageListPageProps {
...
export function GarageListPage(props: GarageListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'garage_manager.count'],
  ...
  const { data, error, isLoading, mutate } = useGarageFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.garage.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('garage_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'garage_manager',
          header: 'Garage Manager',
          accessorKey: 'garage_manager',
          cell: ({ row: { original: record } }: any) => record?._count?.garage_manager,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/garages/view/${record.id}`} passHref legacyBehavior>
                <Button
